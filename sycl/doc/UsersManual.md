...
**`-fsycl-targets=<T1>[,...,<Tn>]`**
    ...
    * spir64-unknown-unknown - this is the default generic SPIR-V
      target;
    * spir64_x86_64-unknown-unknown - generate code ahead of time
      for x86_64 CPUs;
    * spir64_fpga-unknown-unknown - generate code ahead of time for
      Intel FPGA;
...
**`-fintelfpga`**
    ...
    `-fsycl-targets=spir64_fpga-unknown-unknown -g -MMD` on Linux
    and `-fsycl-targets=spir64_fpga-unknown-unknown -Zi -MMD` on
    Windows.
