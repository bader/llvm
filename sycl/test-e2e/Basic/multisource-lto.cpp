...
#elif INIT_KERNEL
SYCL_EXTERNAL int init(int x) { return x + 42; }
void init_buf(queue &q, buffer<int, 1> &b, range<1> &r, int i) {
...
void calc_buf(queue &q, buffer<int, 1> &a, buffer<int, 1> &b, buffer<int, 1> &c,
  ...
  q.submit([&](handler &cgh) {
    ...
    cgh.parallel_for<class calc>(
        r, [=](id<1> index) { C[index] = A[index] - B[index] + init(A[index] + B[index]); });
  });
}
#endif
...
int main() {
  {
    ...
    auto C = c.get_host_access(read_only);
